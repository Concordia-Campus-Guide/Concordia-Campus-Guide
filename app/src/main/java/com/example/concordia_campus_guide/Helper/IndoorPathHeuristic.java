...
public class IndoorPathHeuristic {
    ...
    public double computeHeuristic(final WalkingPoint currentCoordinate, final WalkingPoint destinationPoint) {
        if (!currentCoordinate.getFloorCode().equalsIgnoreCase(destinationPoint.getFloorCode())) {
            ...
            double x = getEuclideanDistance(currentCoordinate, accessPoint)
                    + getEuclideanDistance(accessPoint, destinationPoint);
            return getEuclideanDistance(currentCoordinate, accessPoint)
                    + getEuclideanDistance(accessPoint, destinationPoint);
        }
        double x = getEuclideanDistance(currentCoordinate, destinationPoint);
        return getEuclideanDistance(currentCoordinate, destinationPoint);
    }

    public WalkingPoint getNearestAccessPointForFloor(final WalkingPoint currentPoint) {
        ...
        WalkingPoint x = getClosestPointToCurrentPointFromList(currentPoint, accessPtList);
